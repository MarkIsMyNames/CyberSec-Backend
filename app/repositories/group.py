...
class SQLGroupRepository:
    ...
    def create_group(
        ...
        name: str,
        creator_id: int,
        initial_members: dict[int, bytes] | None = None,
    ) -> Group:
        ...
        logger.info(
            ...
            name,
            creator_id,
            len(initial_members) if initial_members else 0,
        )
        return group
    ...
    def is_creator(self, group_id: int, user_id: int) -> bool:
        return (
            self._session.scalar(select(Group.creator_id).where(Group.id == group_id))
            == user_id
        )

    def add_member(
        self, group_id: int, requester_id: int, user_id: int, skdm_ciphertext: bytes
    ) -> None:
    ...
    def remove_member(
        ...
        if deleted is None:
            logger.debug(
                "remove_member no-op: user not in group group_id=%d user_id=%d",
                group_id,
                user_id,
            )
            return
        ...
        if current_creator_id == user_id:
            ...
            if new_creator is None:
                ...
                logger.info(
                    "group deleted — creator left with no remaining members group_id=%d",
                    group_id,
                )
                self._session.commit()
                return
            self._session.execute(
                ...
                .values(creator_id=new_creator.user_id)
            )
            logger.info(
        ...
        self._session.execute(
            delete(SenderKeyDistribution).where(
                SenderKeyDistribution.group_id == group_id
            )
        )
        if skdm_ciphertexts:
    ...
    def _store_skdms(self, group_id: int, skdm_ciphertexts: dict[int, bytes]) -> None:
        ...
        logger.info(
            "stored %d SKDMs group_id=%d epoch=%d",
            len(skdm_ciphertexts),
            group_id,
            epoch,
        )

    def pop_skdms_for_user(
        self, user_id: int, group_id: int
    ) -> list[tuple[int, bytes]]:
        rows = self._session.execute(
            delete(SenderKeyDistribution)
            .where(
                SenderKeyDistribution.recipient_id == user_id,
                SenderKeyDistribution.group_id == group_id,
            )
            .returning(
                SenderKeyDistribution.epoch, SenderKeyDistribution.skdm_ciphertext
            )
        ).all()
        self._session.commit()
        if not rows:
            return []
        logger.info(
            "popped %d SKDMs user_id=%d group_id=%d", len(rows), user_id, group_id
        )
    ...
    def store_group_message(
        ...
        self._session.execute(
            insert(GroupMessageReceipt).from_select(
                ...
                select(literal(msg.id), GroupMember.user_id).where(
    ...
    def get_group_messages(self, group_id: int, user_id: int) -> list[GroupMessage]:
        messages = list(
            self._session.scalars(
                ...
                .join(
                    GroupMessageReceipt,
                    (GroupMessageReceipt.message_id == GroupMessage.id)
        ...
        logger.debug(
            "fetched %d group messages group_id=%d user_id=%d",
            len(messages),
            group_id,
            user_id,
        )
    ...
    def revoke_group_message(self, message_id: int, sender_id: int) -> bool:
        ...
        if deleted is None:
            logger.warning(
                "group message revoke failed message_id=%d requester_id=%d",
                message_id,
                sender_id,
            )
            return False
        self._session.commit()
        logger.info(
            "group message revoked message_id=%d sender_id=%d", message_id, sender_id
        )
        return True
    ...
    def record_group_receipt(self, message_id: int, user_id: int) -> None:
        ...
        self._session.execute(
            delete(GroupMessage).where(
                ...
                ~exists(
                    select(GroupMessageReceipt.message_id).where(
                        GroupMessageReceipt.message_id == message_id
                    )
                ),
            )
        ...
        self._session.commit()
        logger.debug(
            "group receipt recorded message_id=%d user_id=%d", message_id, user_id
        )
