...
def test_parse_auditd_canary():
    ...
    assert result == AuditEvent(path="/home/student/CyberSec-Backend/.env", principal="1000", agent="1234")
...
def test_parse_auditd_vault_credentials():
    ...
    result = parse_auditd_event(line)
    assert result == AuditEvent(path="/etc/securemsg/vault-credentials", principal="0", agent="5678")
...
def test_parse_vault_event_match():
    ...
    assert result == AuditEvent(
        ...
        principal="approle:securemsg", agent="abc123",
    )
...
def test_parse_vault_event_match_missing_auth():
    ...
    line = json.dumps(entry)
    result = parse_vault_event(line)
    assert result == AuditEvent(path="secret/data/securemsg/prod", principal="", agent="")
